...
from app.main.modules.auth.schemas import LoginSchema
...
class AuthService(GenericService):
    @classmethod
    def validate_login(cls, login_schema_data: LoginSchema):
        ...
        if user_find_by_document:
            ...
            if user and check_password_hash(user.password, login_schema_data.get('password', '')):
                login_dict_response = dict(username=user.name, id=user.id, profile=user.role)
